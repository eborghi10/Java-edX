...
public class Main {
    ...
    public static void runExercise4() {
        students.stream()
                .mapToInt((students)->students.getAge())
                .average()
    ...
    public static void runExercise7() {
        students.stream()   // Manage list with Functional Programming without modifying it
                .filter(student -> student.getAge() > 20)   // Only keep students older than 20 years
                .map(student -> student.getLastName() + ", " + student.getFirstName()) // Order the resultant list
                .sorted()   // Sort as "map()ped"
                .forEach(System.out::println);  // Print sorted() list
    }

    //Make a list of objects type integer with the different years when students were born, sorted descendantly.
    //Hint: distinct reverseOrder collect
    public static void runExercise8() {
        List <Integer> years = students.stream()
                .map(students->students.getBirthYear())
